...
//==================================================================
//unsigned int min_obj(datatype **POP,unsigned int obj_index,node **head,node **mh)
...
//==================================================================
//void  CornerSort(datatype **POP,unsigned int m,unsigned int n,unsigned int *rank)
	...
	while(head!=NULL)
	{
		...
		while(mh!=NULL)//until all the solutions are marked
		{
			for(obj_index=0;mh!=NULL&&obj_index<m;++obj_index)
			{
				...
				cur=min_obj(POP,obj_index,&head,&mh);// delete it in both mark and rank linked lists
				rank[cur]=r;
				cout++;
				p=mh;
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) 
{
	if (nrhs != 3)
    {
        mexErrMsgTxt("r参数个数不正确!");
    }
    if (nlhs != 3)
    {
        mexErrMsgTxt("l参数个数不正确!");
    }

	int n = mxGetN(prhs[0]);
	double* pop = mxGetPr(prhs[0]);
    int m = mxGetPr(prhs[1]); 
    double* rank = mxGetPr(prhs[3]); 
    int M = mxGetM(prhs[3]);
 	int N = mxGetN(prhs[3]);
 	plhs[0] = mxCreateDoubleMatrix(M, N, mxREAL);
 	double* out = mxGetPr(plhs[0]); 
 	out = CornerSort(*pop,m,n,rank);
}